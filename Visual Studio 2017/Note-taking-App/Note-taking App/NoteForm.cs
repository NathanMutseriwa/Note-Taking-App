...
namespace NoteTakingApp
{
    public partial class NoteForm : Form
    {
        ...
        {
            string folderPath = @"C:\YourAppData\Notes"; // Specify your desired folder path
            Directory.CreateDirectory(folderPath);

            string noteText = "This is the content of my note.";
            string noteFileName = "note1.txt"; // You can use a unique filename for each note
            string filePath = Path.Combine(folderPath, noteFileName);

            using (StreamWriter writer = new StreamWriter(filePath))
            {
                writer.WriteLine(noteText);
            }
            string readFilePath = Path.Combine(folderPath, noteFileName);

            if (File.Exists(readFilePath))
            {
                using (StreamReader reader = new StreamReader(readFilePath))
                {
                    string noteContent = reader.ReadToEnd();
